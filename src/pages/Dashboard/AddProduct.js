...
import { addProduct, togglePostProduct } from "../../features/products/productSlice";
...
const AddProduct = () => {
  ...
  const { isLoading, isError, postProduct, error } = useSelector(state => state.products)
  ...
  useEffect(() => {
    ...
    if (!isLoading && postProduct) {
      toast.success('Product added', { id: 'postProduct' });
      reset();
      dispatch(togglePostProduct())
    }

    if (isError && !postProduct) {
      toast.error(error, { id: 'postProduct' });
    }
  }, [isLoading, isError, postProduct, error])
